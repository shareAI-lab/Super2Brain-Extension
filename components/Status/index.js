...
import { getTaskList, updateTaskList, getUserInput } from '../../public/storage'

const GetTaskStatus = 'https://api.super2brain.com/common/tasks/{task_id}'
	...
	return (
		<div className="max-w-[1000px] w-full p-4">
			<div className="bg-white rounded-lg shadow-xl p-4">
					...
					<div className="mt-4 pt-4 border-t border-gray-100">
						...
						<div className="overflow-x-auto">
							...
							<div className="mt-4 flex items-center justify-between">
								<div className="text-sm text-gray-700">共 {taskList.length} 条记录</div>
								<div className="flex items-center space-x-2">
									...
									<span className="text-sm text-gray-700">
										第 {currentPage} 页 / 共 {Math.ceil(taskList.length / pageSize)} 页
									</span>
									<button
										onClick={() =>
											setCurrentPage(prev =>
												Math.min(prev + 1, Math.ceil(taskList.length / pageSize))
											)
										}
										disabled={currentPage >= Math.ceil(taskList.length / pageSize)}
										className={`px-3 py-1 rounded ${
				...
				{hasError && <div className="mt-4 text-red-500 text-sm">处理过程中出现错误,请重试</div>}
			</div>
		</div>
	)
}
