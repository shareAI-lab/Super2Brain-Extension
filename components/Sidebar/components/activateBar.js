const ActivateBar = ({ activatePage, setActivatePage }) => {
	return (
		<div className="flex flex-col items-center space-y-2 pt-4 relative">
			...
			<button
				className="hover:bg-gray-100 rounded-r-full p-2 relative z-10"
				onClick={() => setActivatePage(0)}>
				<MessageCircle
					className={`w-5 h-5 transition-colors duration-200 ease-in-out
          ${activatePage === 0 ? 'text-purple-600' : 'text-gray-600'}`}
				/>
			</button>
			<button
				className="hover:bg-gray-100 rounded-r-full p-2 relative z-10"
				...
				<Network
					className={`w-5 h-5 transition-colors duration-200 ease-in-out
          ${activatePage === 1 ? 'text-purple-600' : 'text-gray-600'}`}
				/>
			</button>
			<button
				className="hover:bg-gray-100 rounded-r-full p-2 relative z-10"
				onClick={() => setActivatePage(2)}>
				<BookOpen
					className={`w-5 h-5 transition-colors duration-200 ease-in-out
          ${activatePage === 2 ? 'text-purple-600' : 'text-gray-600'}`}
				/>
			</button>
			<button
				className="hover:bg-gray-100 rounded-r-full p-2 relative z-10"
				onClick={() => setActivatePage(3)}>
				<Send
					className={`w-5 h-5 transition-colors duration-200 ease-in-out
          ${activatePage === 3 ? 'text-purple-600' : 'text-gray-600'}`}
				/>
			</button>
			<button
				className="hover:bg-gray-100 rounded-r-full p-2 relative z-10"
